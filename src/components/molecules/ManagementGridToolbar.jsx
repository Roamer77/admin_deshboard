...
const ManagementGridToolbar = ({isSelectedRecord, handleRowEdit, addOperationMenu, handleSearchEvent}) => {
    ...
    return (<Toolbar>
        ...
        <div className={classes.searchLine}>
            <InputGroup size='sm'>
                <InputGroupAddon addonType='prepend'>
                    <InputGroupText onClick={() => handleSearchEvent(searchInput)}><FontAwesomeIcon icon={faSearch}
                                                                                                    size="xs"/></InputGroupText>
                </InputGroupAddon>
        ...
        <Button onClick={handleAddProductEvent} hidden={toolBarBtns[0].hidden} color="danger"
                size="sm">{toolBarBtns[0].title}</Button>
        <Button onClick={handleDeleteProductEvent} hidden={toolBarBtns[1].hidden} color="danger"
                size="sm">{toolBarBtns[1].title}</Button>
        {isSelectedRecord &&
        <UpdateModal hidden={!isSelectedRecord} buttonLabel={'Update'} dataForFields={handleRowEdit}/>}
        {modal && <CreateProductForm isOpen={!modal} invertOpenModalCallback={setModal}/>}
    </Toolbar>);
};
